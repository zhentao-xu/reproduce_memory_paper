"""PPO trainer for the Answer Agent.

Mirrors PPO Manager but the reward is direct EM(pred, gold).
"""
...
from memory_r1.prompts.answer import ANSWER_AGENT_SYSTEM
from memory_r1.training.config import TrainerConfig
from memory_r1.training.grpo_answer import AnswerExample
from memory_r1.training.grpo_manager import _cyclic, _response_logprobs
from memory_r1.training.ppo_manager import ValueHead
from memory_r1.training.reward_pipeline import score_answer_batch
from memory_r1.training.rollout import build_chat_prompt
from memory_r1.utils import logger, resolve_device, resolve_dtype



class PPOAnswerTrainer:
    def __init__(self, cfg: TrainerConfig, reward_fn, examples: list[AnswerExample]) -> None:
        self.cfg = cfg
        self.reward_fn = reward_fn
        self.examples = examples

        logger.info("🧠 loading Answer Agent backbone (PPO): {}", cfg.model.name_or_path)
    ...
    def _value_from_response(self, prompt: str, response: str) -> torch.Tensor:
        ...
        return self.value_head(last_hidden).squeeze(0)
    ...
    def train(self) -> None:
        ...
        for step in range(1, cfg.rl.total_steps + 1):
            t0 = time.time()
            batch = [next(it) for _ in range(cfg.optim.total_batch_size // 8)]
            step_stats: dict[str, float] = {}
            for ex in batch:
                prompt = build_chat_prompt(
                    self.tokenizer, ANSWER_AGENT_SYSTEM, ex.render_user_prompt()
                )
                response = self._sample_one(prompt)
                rewards = score_answer_batch(self.reward_fn, [response], [ex.sample], max_workers=1)
