"""Verify that the local LLaMA-3.1-8B-Instruct checkpoint loads and generates coherently.

Usage:
    python scripts/verify_llama_8b.py
    python scripts/verify_llama_8b.py --model-dir /custom/path/to/llama-3.1-8b

Passes if:
    - Config declares ``model_type=llama`` with the expected 32-layer / 4096-hidden shape
...
LLaMA-3.1-8B is the paper's Table-1 SOTA backbone. On an 80 GB H100 the model + KV
cache fits comfortably in bf16 (~16 GB weights + a few GB activations).
"""
...
DEFAULT_MODEL_DIR = (
    Path(__file__).resolve().parent.parent / "models" / "llama-3.1-8b"
)
EXPECTED_MODEL_TYPE = "llama"
EXPECTED_HIDDEN_SIZE = 4096
EXPECTED_NUM_LAYERS = 32

# Short prompt (sanity) + a long one that stresses long-context decode — same LoCoMo-flavored
# prompt used across the verify_* scripts so bugs surfaced by one show up in the others.
...
def main() -> int:
    ...
    print("\n✅ verification passed — LLaMA-3.1-8B loads, config matches, generation is numerically stable.")
    return 0
